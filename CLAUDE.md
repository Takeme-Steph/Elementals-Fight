...
- `Assets/Scripts/CharacterSelect/` — character-select screen (`CharacterSelectController.cs` is the phase machine; replaced the old `PlayerSelection.cs`, which no longer exists — don't recreate it or reference it). Stores picks via `PlayerPrefs` (`selectedCharacter`, `selectedOpponent`) before loading the fight scene, same as before. Reads the cast from `Assets/Scripts/Roster/` (`CharacterId`, `CharacterDefinition`, `CharacterRoster`) and `Assets/Data/Roster/CharacterRoster.asset` — adding a fighter to the select screen is a data change (new `CharacterDefinition` asset appended to the roster), not a script change. `Assets/Editor/CharacterSelect/CharacterSelectSceneBuilder.cs` (menu: Elementals Fight > Character Select > 3 - Build Scene) is the supported way to change the select screen's layout; hand edits to its generated hierarchy are lost on the next run.
